...
object BarcodeImageGenerator {
    ...
    private fun createHints(
        ...
        // Start with initial hints
        val baseHints = mutableMapOf(
            EncodeHintType.CHARACTER_SET to "utf-8",
            EncodeHintType.MARGIN to margin
        )
        // Add error correction level if provided
        if (errorCorrectionLevel != null) {
            baseHints[EncodeHintType.ERROR_CORRECTION] = errorCorrectionLevel
        }
        return baseHints
    }

    private fun createSvg(width: Int, height: Int, matrix: BitMatrix): String {
        val result = StringBuilder().append("<svg width=\"$width\" height=\"$height\"")
