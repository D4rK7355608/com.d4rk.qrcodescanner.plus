...
class ScanBarcodeFromFileActivity : AppCompatActivity() {
    ...
    private fun handleImageCropAreaChanged() {/*binding.cropImageView.touches()
    ...
    private fun scanCroppedImage(image: Bitmap) {
        // Clear previous disposable equivalent logic if needed
        lastScanResult = null
        CoroutineScope(Dispatchers.IO).launch {
            try {
                // Assuming a suspend variant or blocking call of barcodeImageScanner.parse
                val scanResult = barcodeImageScanner.parse(image)
    ...
    private fun saveScanResult() {
        ...
        if (!settings.saveScannedBarcodesToHistory) {
