...
class ExportHistoryActivity : BaseActivity() {
    ...
    private fun exportHistory() {
        ...
        val saveFunc: suspend (Context, String, List<ExportBarcode>) -> Unit = when (binding.spinnerExportAs.selectedItemPosition) {
            0 -> { ctx, name, barcodes -> barcodeSaver.saveBarcodeHistoryAsCsv(ctx, name, barcodes) }
            1 -> { ctx, name, barcodes -> barcodeSaver.saveBarcodeHistoryAsJson(ctx, name, barcodes) }
            else -> {
                return
            }
        }
        ...
        lifecycleScope.launch {
            try {
                // Fetch barcodes for export on IO dispatcher
                val barcodes = withContext(Dispatchers.IO) {
                    barcodeDatabase.getAllForExport()
                }
                // Save the history file on IO dispatcher
                withContext(Dispatchers.IO) {
                    saveFunc(this@ExportHistoryActivity, fileName, barcodes)
                }
