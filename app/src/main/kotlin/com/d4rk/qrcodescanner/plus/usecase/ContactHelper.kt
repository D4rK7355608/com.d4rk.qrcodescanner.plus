...
import com.d4rk.qrcodescanner.plus.data.model.Contact
...
object ContactHelper {
    ...
    fun getPhone(context : Context , result : Intent?) : String? {
        ...
        val cursor =
                contentResolver.query(uri , PHONE_PROJECTION , null , null , null) ?: return null
    ...
    fun getContact(context : Context , result : Intent?) : Contact? {
        ...
        val contentResolver = context.contentResolver
        val cursor =
                contentResolver.query(uri , CONTACT_PROJECTION , null , null , null) ?: return null
        if (cursor.moveToNext().not()) {
            cursor.close()
            return null
        }
    ...
    private fun buildContactPhoneDetails(
        ...
        if (cursor.getStringOrNull(ContactsContract.Contacts.HAS_PHONE_NUMBER)?.toInt()
                    .orZero() <= 0
        ) {
            cursor.close()
            return
        ...
        contact.middleName =
                cursor.getStringOrNull(ContactsContract.CommonDataKinds.StructuredName.MIDDLE_NAME)
        contact.lastName =
                cursor.getStringOrNull(ContactsContract.CommonDataKinds.StructuredName.FAMILY_NAME)
    ...
    private fun buildEmailDetails(
        ...
        val emailWhere =
                ContactsContract.Data.LOOKUP_KEY + " = ? AND " + ContactsContract.Data.MIMETYPE + " = ?"
        val emailWhereParams =
                arrayOf(lookupKey , ContactsContract.CommonDataKinds.Email.CONTENT_ITEM_TYPE)
        val cursor = contentResolver.query(
    ...
    private fun buildAddressDetails(
        ...
        contact.poBox =
                cursor.getStringOrNull(ContactsContract.CommonDataKinds.StructuredPostal.POBOX)
        contact.street =
                cursor.getStringOrNull(ContactsContract.CommonDataKinds.StructuredPostal.STREET)
        contact.city =
                cursor.getStringOrNull(ContactsContract.CommonDataKinds.StructuredPostal.CITY)
        contact.state =
